...
function ensureProductsTable(): void
{
    ...
    // Try to add stock_quantity if it missing (in case table existed but old schema)
    try {
        db()->exec('ALTER TABLE products ADD COLUMN stock_quantity INT DEFAULT 0 AFTER stock_status');
    } catch (Throwable $e) {
...
function createProduct(array $productData): int
{
    ...
    $stmt = db()->prepare(
        ...
        (name, description, price, category, image, stock_status, stock_quantity)
        VALUES 
        (:name, :description, :price, :category, :image, :stock_status, :stock_quantity)'
    );
...
function updateProduct(int $id, array $productData): bool
{
    ...
    if ($image !== null) {
        $stmt = db()->prepare(
            'UPDATE products SET name = :name, description = :description, price = :price, category = :category, image = :image, stock_status = :stock_status, stock_quantity = :stock_quantity WHERE id = :id'
        );
        $params = [
            'name' => $name,
            'description' => $description,
        ...
        $stmt = db()->prepare(
            'UPDATE products SET name = :name, description = :description, price = :price, category = :category, stock_status = :stock_status, stock_quantity = :stock_quantity WHERE id = :id'
        );
        $params = [
            'name' => $name,
            'description' => $description,
