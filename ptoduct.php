			...
			<article class="panel">
				...
				<div class="badge-row">
					...
					<?php if (($product['stock_status'] ?? 'in_stock') === 'in_stock'): ?>
						<span class="badge"><?php echo (int)($product['stock_quantity'] ?? 0); ?> available</span>
					<?php endif; ?>
				...
				<div style="display: flex; gap: 8px; flex-wrap: wrap; align-items: center;">
					<div style="display: flex; align-items: center; gap: 8px; border: 1px solid var(--border); border-radius: 8px; padding: 0 8px; opacity: <?php echo $isOutOfStock ? '0.5' : '1'; ?>;">
						...
						<input type="number" id="qty-input" value="1" min="1" max="<?php echo (int)($product['stock_quantity'] ?? 1); ?>" style="width: 50px; text-align: center; border: none; background: none;" <?php echo $isOutOfStock ? 'disabled' : ''; ?> />
						<button type="button" onclick="<?php echo $isOutOfStock ? 'return false;' : "var val = parseInt(document.getElementById('qty-input').value); var max = parseInt(document.getElementById('qty-input').max); if(val < max) document.getElementById('qty-input').value = val + 1;"; ?>" class="btn" style="background: none; border: none; padding: 8px; cursor: <?php echo $isOutOfStock ? 'not-allowed' : 'pointer'; ?>; font-size: 1.2rem;" <?php echo $isOutOfStock ? 'disabled' : ''; ?>>+</button>
					</div>
					<?php if ($isOutOfStock): ?>
				...
				<ul class="stack-list">
					...
					<li><span>Status</span><strong><?php echo ($product['stock_status'] ?? 'in_stock') === 'in_stock' ? 'In stock (' . (int)($product['stock_quantity'] ?? 0) . ')' : 'Out of stock'; ?></strong></li>
